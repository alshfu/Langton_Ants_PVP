...
class Inbox {
    ...
    next(timeoutMs = 2000) {
        if (this.msgs.length > 0)
            return Promise.resolve(this.msgs.shift());
...
describe('MvpServer integration', () => {
    ...
    const joinMsg = (room, nick, locale = 'en') => JSON.stringify({ type: 'join_room', roomCode: room, nickname: nick, locale });
    // ─── Базовые (Day 2) ───────────────────────────────────────────────────────
    ...
    it('error message локализован (ru)', async () => {
        ...
        expect(err.locale).toBe('ru');
        expect(err.message).toContain('Матч');
        ws.close();
    });
    // ─── Day 3 — Room flow ────────────────────────────────────────────────────
    it('two clients join same room → оба видят друг друга', async () => {
        ...
        expect(ackB.players).toHaveLength(2);
        // A получает room_updated после прихода B
        const updA = await a.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 2);
        expect(updA.players[0].nickname).toBe('Alice');
        expect(updA.players[1].nickname).toBe('Bob');
        a.ws.close();
        b.ws.close();
    });
    ...
    it('set_ready broadcasts другому игроку', async () => {
        ...
        b.ws.send(joinMsg('r', 'B'));
        await b.inbox.readN(2);
        await a.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 2);
        a.ws.send(JSON.stringify({ type: 'set_ready', ready: true }));
        const updA = await a.inbox.waitFor((m) => m.type === 'room_updated' && m.players.find((p) => p.nickname === 'A')?.ready === true);
        const updB = await b.inbox.waitFor((m) => m.type === 'room_updated' && m.players.find((p) => p.nickname === 'A')?.ready === true);
        expect(updA.players.find((p) => p.nickname === 'A').ready).toBe(true);
        expect(updB.players.find((p) => p.nickname === 'B').ready).toBe(false);
        a.ws.close();
        b.ws.close();
    });
    it('disconnect → партнёр получает room_updated с 1 игроком', async () => {
        ...
        await a.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 2);
        a.ws.close();
        const updB = await b.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 1);
        expect(updB.players[0].nickname).toBe('B');
    ...
    it('server.stop() closes all connections', async () => {
        ...
        const closePromiseA = new Promise((res) => a.ws.once('close', () => res()));
        const closePromiseB = new Promise((res) => b.ws.once('close', () => res()));
        await server.stop();
    ...
    async function setupReadyPair(roomCode = 'r') {
        ...
        await a.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 2);
        await b.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 2);
        a.ws.send(JSON.stringify({ type: 'set_ready', ready: true }));
    ...
    it('оба ready → match_starting broadcast (config, seed, matchId)', async () => {
        ...
        expect(startingA.matchId).toBeDefined();
        expect(startingA.seed).toEqual(startingB.seed); // оба видят тот же seed
        expect(startingA.config.width).toBe(60);
        expect(startingA.countdownMs).toBe(30);
        a.ws.close();
        b.ws.close();
    });
    it('после countdown → match_started', async () => {
        ...
        expect(startedA.matchId).toEqual(startedB.matchId);
        expect(startedA.serverEngineVersion).toBe('0.1.0');
        a.ws.close();
        b.ws.close();
    });
    it('после match_started → match_tick broadcast', async () => {
        ...
        expect(tickA.tick).toBeGreaterThanOrEqual(1);
        expect(tickB.tick).toBeGreaterThanOrEqual(1);
        expect(tickA.deploys).toEqual([]); // Day 4 — пустой
        a.ws.close();
        b.ws.close();
    });
    it('time win → match_ended после threshold ticks', async () => {
        ...
        expect(endedA.result.finished).toBe(true);
        expect(endedA.result.reason).toBe('time_expired');
        expect(endedB.result.finished).toBe(true);
        expect(endedA.result.finishedAtTick).toBeGreaterThanOrEqual(300);
        a.ws.close();
        b.ws.close();
    });
    ...
    it('disconnect во время countdown → countdown cancel, partner stays in lobby', async () => {
        ...
        await b.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 1, 1000);
    ...
    it('disconnect во время match → match_ended с winner=другой', async () => {
        ...
        expect(ended.result.winnerId).toBe('p1'); // B = index 1, побеждает
        b.ws.close();
    });
    // ─── Day 5 — Deploy ────────────────────────────────────────────────────────
    it('deploy до match_started → MATCH_NOT_ACTIVE', async () => {
        ...
        expect(err.code).toBe('MATCH_NOT_ACTIVE');
        a.ws.close();
    });
    it('valid deploy во время match → broadcast в match_tick.deploys', async () => {
        ...
        const curT = firstTick.tick;
        a.ws.send(JSON.stringify({ type: 'deploy', x: 30, y: 30, tick: curT }));
        // Оба получают match_tick с deploy
        const tickWithDeploy = await b.inbox.waitFor((m) => m.type === 'match_tick' && m.deploys.length > 0, 1000);
        expect(tickWithDeploy.deploys[0].x).toBe(30);
        expect(tickWithDeploy.deploys[0].y).toBe(30);
        expect(tickWithDeploy.deploys[0].playerIdx).toBe(0); // A = index 0
        a.ws.close();
        b.ws.close();
    });
    ...
    it('двойной deploy на ту же клетку → второй INVALID_DEPLOY (occupied)', async () => {
        ...
        a.ws.send(JSON.stringify({ type: 'deploy', x: 30, y: 30, tick: curT }));
        await a.inbox.waitFor((m) => m.type === 'match_tick' && m.deploys.some((d) => d.x === 30 && d.y === 30), 500);
        ...
        const reply = await Promise.race([
            ...
            b.inbox.waitFor((m) => m.type === 'match_tick' && m.deploys.some((d) => d.playerIdx === 1 && d.x === 30 && d.y === 30), 300).catch(() => null),
        ]);
    ...
    it('очень старый tick → INPUT_TOO_OLD', async () => {
        ...
        a.ws.send(JSON.stringify({ type: 'deploy', x: 30, y: 30, tick: 0 }));
        const err = await a.inbox.waitFor((m) => m.type === 'error', 500);
        expect(err.code).toBe('INPUT_TOO_OLD');
        a.ws.close();
        b.ws.close();
    });
});
...
describe('MvpServer · Day 13 reconnect', () => {
    ...
    const joinMsg = (room, nick, opts = {}) => JSON.stringify({
        type: 'join_room',
        roomCode: room,
        nickname: nick,
    ...
    async function setupActiveMatch(roomCode = 'r') {
        ...
        return {
            a, b,
            tokenA: joinedA.resumeToken,
            tokenB: joinedB.resumeToken,
        };
    }
    it('room_joined содержит resumeToken (32 hex)', async () => {
        ...
        expect(joined.resumeToken).toMatch(/^[0-9a-f]{32}$/);
        expect(joined.resumed).toBeUndefined();
        a.ws.close();
    });
    it('reconnect с valid token → resumed:true + match_resume_state', async () => {
        ...
        expect(rejoined.resumed).toBe(true);
        expect(rejoined.clientId).toBeDefined();
        // Получаем match_resume_state с polynomial data
        const resume = await a2.inbox.waitFor((m) => m.type === 'match_resume_state', 500);
        expect(resume.matchId).toBeDefined();
        expect(resume.tick).toBeGreaterThan(0);
        expect(resume.config).toBeDefined();
        expect(resume.seed).toBeDefined();
        expect(Array.isArray(resume.deployTimeline)).toBe(true);
        a2.ws.close();
        b.ws.close();
    });
    ...
    it('opponent видит disconnected flag в room_updated', async () => {
        ...
        a.ws.close();
        const update = await b.inbox.waitFor((m) => m.type === 'room_updated'
            && m.players.some((p) => p.disconnected === true), 500);
        const ghostPlayer = update.players.find((p) => p.disconnected);
    ...
    it('reconnect → opponent видит disconnected=false снова', async () => {
        const { a, b, tokenA } = await setupActiveMatch();
        a.ws.close();
        await b.inbox.waitFor((m) => m.type === 'room_updated'
            && m.players.some((p) => p.disconnected), 500);
        const a2 = await openClient(url);
        a2.ws.send(joinMsg('r', 'Alice', { resumeToken: tokenA }));
        await a2.inbox.waitFor((m) => m.type === 'room_joined');
        const update = await b.inbox.waitFor((m) => m.type === 'room_updated'
            && m.players.every((p) => !p.disconnected), 500);
    ...
    it('disconnect in lobby → immediate (no grace, opponent stays)', async () => {
        ...
        await a.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 2);
        a.ws.close();
        const update = await b.inbox.waitFor((m) => m.type === 'room_updated' && m.players.length === 1, 500);
        expect(update.players[0].nickname).toBe('B');
        b.ws.close();
    });
});
