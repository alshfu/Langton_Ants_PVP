 ...
 */
export function startMatchCountdown(room, ctx) {
    if (room.activeMatch)
        return false;
    ...
    if (config.width > PVP_MAX_FIELD || config.height > PVP_MAX_FIELD) {
        for (const p of room.players)
            p.sendError(ERROR_CODES.FIELD_TOO_LARGE_FOR_PVP);
        return false;
    }
...
export function cancelMatchCountdown(room) {
    if (room.countdownHandle) {
...
export function endActiveMatch(room, reason, winnerIndex = null) {
    if (!room.activeMatch)
        return;
...
// ─── private ─────────────────────────────────────────────────────────────────
function finalizeMatchStart(room, config, matchId, ctx) {
...
//# sourceMappingURL=matchLifecycle.js.map