...
import { ERROR_CODES, isClientMessage } from './messages.js';
import { isValidNickname } from './nicknames.js';
import { startMatchCountdown, cancelMatchCountdown, endActiveMatch, } from './matchLifecycle.js';
/**
 * Парсит raw buffer/string как JSON, валидирует shape, передаёт в handler.
 * Любая ошибка → conn.sendError(...) и return (не throw).
 */
export function routeMessage(conn, raw, ctx) {
    let parsed;
    try {
...
function dispatch(conn, msg, ctx) {
    switch (msg.type) {
...
// ─── Handlers ────────────────────────────────────────────────────────────────
function handleJoinRoom(conn, msg, ctx) {
...
function handleLeaveRoom(conn, ctx) {
    leaveCurrentRoom(conn, ctx);
}
function handleSetReady(conn, msg, ctx) {
    if (!conn.roomCode) {
...
function handleDeploy(conn, msg, ctx) {
    if (!conn.roomCode) {
        conn.sendError(ERROR_CODES.NOT_IN_ROOM);
        return;
    }
 ...
 */
export function leaveCurrentRoom(conn, ctx) {
    if (!conn.roomCode)
        return;
    const room = ctx.rooms.get(conn.roomCode);
    if (!room) {
 ...
 */
export function handleConnectionClose(conn, ctx) {
    if (!conn.roomCode)
        return false;
    const room = ctx.rooms.get(conn.roomCode);
    if (!room) {
