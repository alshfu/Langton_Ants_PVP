 ...
 */
export function formatTimer(ticks, tps = 10) {
 ...
 *   formatSrDelta(0)   → "±0"
 */
export function formatSrDelta(delta) {
    if (delta === 0)
        return '±0';
 ...
 * Соответствие: §7.6.
 */
export function pingCategory(ms) {
    if (ms <= 30)
        return { color: '#39D98A', bars: 5, label: 'excellent' };
    if (ms <= 80)
        return { color: '#FFD60A', bars: 4, label: 'good' };
    if (ms <= 150)
        return { color: '#FF8A3D', bars: 3, label: 'fair' };
    if (ms <= 250)
        return { color: '#FF453A', bars: 2, label: 'poor' };
