...
// ─────────────────────────────────────────────────────────────────────────────
// Правила движения (расширенная таблица)
// ─────────────────────────────────────────────────────────────────────────────
...
export interface RuleMeta {
  ...
  color: string;
  description: string;
  cost: number;            // в очках отряда для драфта
  isCustom: boolean;
}
 ...
 */
export function rankFromSr(sr: number): RankTier {
  for (const tier of RANK_TIERS) {
    if (sr >= tier.minSr && sr < tier.maxSr) return tier;
  }
