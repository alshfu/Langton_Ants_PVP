 ...
 */
export function validateMessage(msg: unknown): msg is { type: string; seq: number; ts: number; payload: unknown } {
  if (!msg || typeof msg !== 'object') return false;
  const type = (msg as { type: unknown }).type;
  if (typeof type !== 'string') return false;
  const validator = validators[type];
  if (!validator) return false;       // неизвестный type → невалидно
  return validator(msg) as boolean;
}
