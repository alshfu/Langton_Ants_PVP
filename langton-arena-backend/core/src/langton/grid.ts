...
const SQUARE_NEIGHBORS: Offsets = [
  [0, -1], [1, 0], [0, 1], [-1, 0],
];
...
const TRIANGLE_NEIGHBORS_UP: Offsets = [
  [-1, 0], // 0: left
  [1, 0],  // 1: right
  [0, 1],  // 2: down (across base edge of up-triangle)
];
const TRIANGLE_NEIGHBORS_DOWN: Offsets = [
  [-1, 0], // 0: left
  [1, 0],  // 1: right
...
const HEX_NEIGHBORS_EVEN_ROW: Offsets = [
  [0, -1],   // 0: N
...
const HEX_NEIGHBORS_ODD_ROW: Offsets = [
  [0, -1],   // 0: N
  [1, 0],    // 1: NE
  [1, 1],    // 2: SE
  [0, 1],    // 3: S
 ...
 */
export function getNeighbors(x: number, y: number, gridType: GridType): Offsets {
  switch (gridType) {
    case 'square':
...
export function getNumDirs(gridType: GridType): number {
  switch (gridType) {
 ...
 */
export function applyRuleChar(dir: number, ch: string, numDirs: number): number {
  switch (ch) {
